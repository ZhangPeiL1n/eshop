...
public class PurchaseOrderItemDAOTest {
    ...
    public void testBatchSave() throws Exception {
        ...
        Assert.assertEquals((int) count, actualPurchaseOrderItems.size());
    }
    ...
    public void testRemoveByPurchaseOrderId() throws Exception {
        ...
        Assert.assertEquals(0, actualPurchaseOrderItems.size());
    }

    /**
     * 比较两个采购单条目集合
     *
     * @param expectedPurchaseOrderItems 期望的采购单条目集合
     * @param actualPurchaseOrderItems   实际的采购单条目集合
     * @throws Exception
     */
    private void comparePurchaseOrderItem(
        ...
        Assert.assertEquals((int) expectedSize, actualPurchaseOrderItems.size());
        for (PurchaseOrderItemDO actualPurchaseOrderItem : actualPurchaseOrderItems) {
            ...
            Assert.assertEquals(expectedPurchaseOrderItem, actualPurchaseOrderItem);
        }
    }
    ...
    private Map<Long, PurchaseOrderItemDO> createPurchaseOrderItemMap(
        ...
        Map<Long, PurchaseOrderItemDO> purchaseOrderItemMap = new HashMap<>();
    ...
    private List<PurchaseOrderItemDO> createPurchaseOrderItems(
            Integer count, Long purchaseOrderId) throws Exception {
        List<PurchaseOrderItemDO> purchaseOrderItemList = new ArrayList<>();
