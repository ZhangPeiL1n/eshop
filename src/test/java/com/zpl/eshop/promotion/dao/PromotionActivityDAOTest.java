...
public class PromotionActivityDAOTest {
    ...
    public void testSave() throws Exception {
        ...
        Assert.assertThat(activity.getId(), Matchers.greaterThan(0L));
    }
    ...
    public void testListByPage() throws Exception {
        ...
        Map<Long, PromotionActivityDO> expectedActivityMap = createPromotionActivityMap(count, includedRule);

        Integer expectedSize = 10;
        PromotionActivityQuery query = createPromotionActivityQuery(expectedSize);
        List<PromotionActivityDO> actualActivities = promotionActivityDAO.listByPage(query);
    ...
    public void testListEnabledByGoodsId() throws Exception {
        ...
        Boolean includedRule = false;
        Map<Long, PromotionActivityDO> expectedActivityMap = createPromotionActivityMap(activityCount, includedRule);

        Long[] goodsIds = new Long[]{1L, -999L};
        createPromotionActivityGoodsRelations(expectedActivityMap, goodsIds);

        Long goodsId = 1L;
        List<PromotionActivityDO> actualActivities = promotionActivityDAO.listEnabledByGoodsId(goodsId);

        Assert.assertEquals((int) activityCount, actualActivities.size());
        ...
        for (PromotionActivityDO actualActivity : actualActivities) {
            ...
            Assert.assertEquals(expectedActivity.getName(), actualActivity.getName());
    ...
    public void testRemove() throws Exception {
        ...
        PromotionActivityDO actualActivity = promotionActivityDAO.getById(expectedActivity.getId());
        Assert.assertNull(actualActivity);
    }
     ...
     */
    private void compareActivities(Integer expectedSize, Map<Long, PromotionActivityDO> expectedActivityMap, List<PromotionActivityDO> actualActivities) {
        Assert.assertEquals((int) expectedSize, actualActivities.size());
        for (PromotionActivityDO actualActivity : actualActivities) {
            PromotionActivityDO expectedActivity = expectedActivityMap.get(actualActivity.getId());
            Assert.assertEquals(expectedActivity, actualActivity);
        }
     ...
     * @throws Exception
     */
    private void createPromotionActivityGoodsRelations(Map<Long, PromotionActivityDO> activityMap, Long[] goodsIds) throws Exception {
        List<PromotionActivityDO> activities = new ArrayList<>(activityMap.values());
     ...
     * @return 促销活动查询条件
     */
    private PromotionActivityQuery createPromotionActivityQuery(Integer expectedSize) {
        ...
        query.setEndTime("2022-01-10 23:59:59");
        return query;
    }
     ...
     * @throws Exception
     */
    private Map<Long, PromotionActivityDO> createPromotionActivityMap(Integer count, Boolean includedRule) throws Exception {
        Map<Long, PromotionActivityDO> activityMap = new HashMap<>();

        List<PromotionActivityDO> activities = createPromotionActivities(count, includedRule);
     ...
     * @throws Exception
     */
    private List<PromotionActivityDO> createPromotionActivities(Integer count, Boolean includedRule) throws Exception {
        List<PromotionActivityDO> activities = new ArrayList<>();
    ...
    private PromotionActivityDO createPromotionActivity() throws Exception {
        ...
        activity.setStartTime(dateProvider.parse2Datetime("2022-01-01 10:00:00"));
        activity.setEndTime(dateProvider.parse2Datetime("2022-01-10 10:00:00"));
